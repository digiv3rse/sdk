...
function erc20(address, provider) {
  return new contracts.Contract(address, erc20Abi, provider);
}
function digiFollowNFT(address, provider) {
  return new contracts.Contract(address, digiFollowNftAbi, provider);
}
function digiHub(address, provider) {
  return new contracts.Contract(address, digiHubAbi, provider);
}
function digiTokenHandleRegistry(address, provider) {
  return new contracts.Contract(address, digiTokenHandleRegistryAbi, provider);
}
function publicActProxy(address, provider) {
  return new contracts.Contract(address, publicActProxyAbi, provider);
}
function permissionlessCreator(address, provider) {
  return new contracts.Contract(address, permissionlessCreatorAbi, provider);
}
 ...
 * ```
 */
function encodeData(abi$1, data) {
  sharedKernel.invariant(abi$1.length === data.length, 'Please provide the same number of data items as required by the contract method');
  var types = abi$1.map(function (param, index) {
    var dataItem = data[index];
    sharedKernel.invariant(dataItem !== undefined, 'Please provide a value for each data item');
    return abi.ParamType.fromObject({
      name: param.name,
      type: param.type,
  ...
  return abi.defaultAbiCoder.encode(types, data);
}
 ...
 * ```
 */
function decodeData(abi$1, encoded) {
  var types = abi$1.map(function (param) {
    return abi.ParamType.fromObject({
      name: param.name,
      type: param.type,
      components: param.components
  ...
  return abi.defaultAbiCoder.decode(types, encoded);
}
...
function bigNumber(from) {
  return units.parseUnits(from.toFixed(), from.asset.decimals);
}

exports.bigNumber = bigNumber;
exports.decodeData = decodeData;
exports.digiFollowNFT = digiFollowNFT;
exports.digiHub = digiHub;
exports.digiTokenHandleRegistry = digiTokenHandleRegistry;
exports.encodeData = encodeData;
exports.erc20 = erc20;
exports.hexToInt = hexToInt;
exports.isTheSameAddress = isTheSameAddress;
exports.isValidHandle = isValidHandle;
exports.permissionlessCreator = permissionlessCreator;
exports.publicActProxy = publicActProxy;

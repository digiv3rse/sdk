...
import {
  ...
  mockDiGiTransactionStatusDataResponse,
  mockDiGiTransactionResult,
...
import { ChainType, InvariantError } from '@digiv3rse/shared-kernel';
    ...
    describe(`with a tx hash`, () => {
      ...
      it('should succeed with indexed=true if the tx has been indexed by the BE', async () => {
        const responses = [
          ...
          mockDiGiTransactionStatusDataResponse({
            request: { forTxHash: txHash },
            result: mockDiGiTransactionResult({
              status: DiGiTransactionStatusType.Complete,
              txHash,
            }),
          }),
        ];
        const apolloClient = mockDiGiApolloClient(responses);
        const transaction = setupTransactionObserver({
          apolloClient,
        });
      ...
      it(`should fail with ${TransactionError.name}[reason: ${TransactionErrorReason.REVERTED}] if the tx gets reverted`, async () => {
        const apolloClient = mockDiGiApolloClient([
          mockDiGiTransactionStatusDataResponse({
            request: { forTxHash: txHash },
            result: mockDiGiTransactionResult({
              status: DiGiTransactionStatusType.Failed,
              reason: DiGiTransactionFailureType.Reverted,
            }),
          }),
        ]);
        const transaction = setupTransactionObserver({

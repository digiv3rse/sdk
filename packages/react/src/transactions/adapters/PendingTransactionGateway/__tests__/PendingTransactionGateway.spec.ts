import { MetaTransaction, TransactionKind } from '@digiv3rse/domain/entities';
import {
...
} from '@digiv3rse/domain/mocks';
import { AnyTransactionRequest } from '@digiv3rse/domain/use-cases/transactions';
import { InvariantError } from '@digiv3rse/shared-kernel';
import { IStorage, Storage } from '@digiv3rse/storage';
import { mockStorage } from '@digiv3rse/storage/mocks';
...
describe(`Given an instance of the ${PendingTransactionGateway.name}`, () => {
  ...
  describe(`when the "${PendingTransactionGateway.prototype.getNextMetaTransactionNonceFor.name}" method is invoked`, () => {
    ...
    describe('for a TransactionKind corresponding to a protocol method exposed on the DiGiHub.sol contract', () => {
      it(`should compute the next Nonce based on the most recent ${MetaTransaction.name} exposed from the same contract`, async () => {
        const gateway = setupPendingTransactionGateway({ factory });

        for (let idx = 0; idx < digiHubTransactionKinds.length; idx++) {
          const kind = digiHubTransactionKinds[idx];
    ...
    describe.skip('for a TransactionKind corresponding to a protocol method exposed on the TokenHandleRegistry.sol contract', () => {
      ...
      it(`should not be affected by ${MetaTransaction.name} for methods exposed on DiGiHub.sol contract`, async () => {
        const request = mockCreatePostRequest();
        const gateway = setupPendingTransactionGateway({ factory });
        const init = mockMetaTransactionData({ request });
        const tx = factory.createMetaTransaction(init);

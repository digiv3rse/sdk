import { profileId, useSession, SessionType, useLazyProfile, useLazyProfiles, useSharedDependencies } from '@digiv3rse/react';
import { invariant, assertError, Deferred } from '@digiv3rse/shared-kernel';
import { useMemo, useEffect, useState, useCallback } from 'react';
import { d as _slicedToArray, e as _arrayLikeToArray, f as _unsupportedIterableToArray, c as _objectSpread2, b as _defineProperty, _ as _createClass, a as _classCallCheck } from '../../dist/createClass-2e838422.esm.js';
import { useStartConversation, useClient, Client } from '@xmtp/react-sdk';
...
function parseConversationId(conversationId) {
  ...
  var _conversationIdWithou = conversationIdWithoutPrefix.split('-'),
    _conversationIdWithou2 = _slicedToArray(_conversationIdWithou, 2),
...
function extractPeerProfileId(conversationId, activeProfileId) {
  if (isDiGiConversation(activeProfileId, conversationId)) {
    var _parseConversationId = parseConversationId(conversationId),
      _parseConversationId2 = _slicedToArray(_parseConversationId, 2),
...
function useEnhanceConversation(_ref) {
  ...
  var peerProfileId = useMemo(function () {
    var _conversation$context;
  ...
  var _useLazyProfile = useLazyProfile(),
  ...
  var enhancedConversation = useMemo(function () {
    if (peerProfile) {
...
function useEnhanceConversations(useConversationsResult) {
  ...
  invariant(session === null || session === void 0 ? void 0 : session.authenticated, 'You must be authenticated to use this operation. Use `useLogin` hook to authenticate.');
  invariant(session.type === SessionType.WithProfile, 'You must have a profile to use this operation.');
  var conversationToProfileIdMap = useMemo(function () {
    return conversations.reduce(function (acc, c) {
      ...
      return _objectSpread2(_objectSpread2({}, acc), {}, _defineProperty({}, createUniqueConversationId(c), peerProfileId));
    }, {});
  }, [conversations, session.profile.id]);
  var uniqueProfileIds = useMemo(function () {
  ...
  var _useLazyProfiles = useLazyProfiles(),
  ...
  var enhancedConversations = useMemo(function () {
...
var SignerAdapter = /*#__PURE__*/function () {
  function SignerAdapter(activeWallet) {
    _classCallCheck(this, SignerAdapter);
    this.activeWallet = activeWallet;
  }
  _createClass(SignerAdapter, [{
    key: "getAddress",
    value: function () {
        ...
        return _regeneratorRuntime().wrap(function _callee2$(_context2) {
          while (1) switch (_context2.prev = _context2.next) {
            case 0:
              ...
              useCase = new SignArbitraryMessage(this.activeWallet, presenter);
              _context2.next = 4;
...
function useXmtpClient() {
  ...
  var initializeWithDiGi = useCallback( /*#__PURE__*/_asyncToGenerator( /*#__PURE__*/_regeneratorRuntime().mark(function _callee() {
    var options,
      existingKeys,
      signer,
      _newClient,
...
export { useEnhanceConversation, useEnhanceConversations, useStartDiGiConversation, useXmtpClient };

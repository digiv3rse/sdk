...
describe('Given a collection of inbox helper functions', () => {
  ...
  describe(`when calling ${extractPeerProfileId.name}`, () => {
    it('should return undefined if not a digi conversation', () => {
      const result1 = extractPeerProfileId('not-digi-conversation', '0x15');

      expect(result1).toBe(undefined);
    });

    it('should return peer profileId for a digi conversation', () => {

...
import { DiGiClient } from '.';
import { buildTestEnvironment } from './__helpers__';

describe(`Given storage and two ${DiGiClient.name} instances sharing the same storage`, () => {
  const storage = new InMemoryStorageProvider();
  const config = {
    environment: buildTestEnvironment(),
    storage,
  };
  const client1 = new DiGiClient(config);
  const client2 = new DiGiClient(config);

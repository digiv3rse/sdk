...
import abi from '../../../abi/DiGiHub.json';
import type { DiGiHub } from '../../../contracts/DiGiHub';
...
async function main() {
  ...
  if (!profile.sponsor) {
    ...
    const digiHub = new ethers.Contract(digiHubAddress.development, typedAbi, wallet) as DiGiHub;

    const tx = await digiHub.setProfileMetadataURI(profile.id, metadataURI);
  ...
  // check if the profile has signless enabled aka. enabled digi profile manager
  if (profile.signless) {

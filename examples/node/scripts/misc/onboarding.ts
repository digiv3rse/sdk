...
import { MetadataAttributeType, profile as createProfileMetadata } from '@digiv3rse/metadata';
...
const HANDLE_NAMESPACE = 'test'; // use 'digi' namespace for production
...
async function main() {
  ...
  const digiRelayerAddress = await client.transaction.generateDiGiAPIRelayAddress();
  ...
  // set the profile metadata on the DiGi protocol
  ...
  if (metadataTxCompletion?.status === DiGiTransactionStatusType.Failed) {
